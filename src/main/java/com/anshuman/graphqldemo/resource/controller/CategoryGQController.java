...
public class CategoryGQController {
    ...
    @BatchMapping(typeName = "Film", value = "filmCategories")
    public Mono<Map<Film, Set<FilmCategory>>> filmCategories(Set<Film> films) {
        ...
        return Mono.fromFuture(filmCategoryService.getCategoriesByFilmIds(filmIds))
                .map(filmCategories -> films.stream()
                        .collect(toMap(film -> film, film -> getMatchingCategoriesByFilmId(filmCategories, film))));
    }
    ...
    @BatchMapping(typeName = "FilmCategory", value = "id")
    ...
    @BatchMapping(typeName = "FilmCategory", value = "category")
    public Mono<Map<FilmCategory, Category>> category(Set<FilmCategory> filmCategories) {
        Set<Short> categoryIds = filmCategories.stream()
                .map(FilmCategory::getId)
                .map(FilmCategoryId::getCategoryId)
                .collect(Collectors.toSet());
        return Mono.fromFuture(categoryService.getCategoriesByIds(categoryIds))
                .map(categories -> filmCategories.stream()
                        .collect(toMap(fc -> fc, fc -> getMatchingCategoriesByCatId(categories, fc))));
    }
